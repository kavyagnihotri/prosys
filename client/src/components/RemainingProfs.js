...
export default function RemainingProfs() {
  ...
  if(change==0){
    ...
    await fetch('/prof/dismiss',{
      method: 'POST', 
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({id: prof._id})
    })
    const fetchProjects = async () => {
      const response = await fetch('/prof/', {
        method: "POST",
  ...
  return (
      ...
      <Table size="small">
        ...
        <TableBody>
          {projects && projects.map((project) => (
            project.hod===false && change===-1 &&
            <TableRow key={project._id}>
              <TableCell>{project.name}</TableCell>
              <TableCell>{project.dept}</TableCell>
              <TableCell><Button size="large" startIcon={<PersonAddAlt1Icon />} type="submit" onClick={(e) => Appoint(project)}>APPOINT AS HOD</Button>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
    ...
    <DialogContent>
      <DialogContentText id="alert-dialog-description">
        {prof1.name} is already HOD for {prof1.dept} department.
        Do you want to dismiss them and appoint {prof2.name} as HOD?
      </DialogContentText>
    </DialogContent>
    <DialogActions>
